...
let rec handle_assert : type d r.
 ...
 fun ~eval_expr ~lang pred m pos ->
  ...
  | ELit (LBool false) ->
    ...
    | _ ->
      (if Global.options.no_fail_on_assert
...
and partially_evaluate_expr_for_assertion_failure_message : type d r.
  ...
  (* Here we want to print an expression that explains why an assertion has
     ...
     the AST while evaluating everything below ; we also skip all true subterms within an [And]. This makes for a good error
     message. *)
  match Mark.remove e with
  | EAppOp { op = And, _; args = [e1; e2]; _ } when not neg ->
    (match eval_expr e1 with
     | ELit (LBool true), _ -> partially_evaluate_expr_for_assertion_failure_message ~eval_expr e2
     | _ -> partially_evaluate_expr_for_assertion_failure_message ~eval_expr e1)
  | EAppOp { op = Or, _; args = [e1; e2]; _ } when neg ->
    (match eval_expr e1 with
     | ELit (LBool false), _ -> partially_evaluate_expr_for_assertion_failure_message ~eval_expr e2
     | _ -> partially_evaluate_expr_for_assertion_failure_message ~eval_expr e1)
  | EAppOp
      {
        args = [e1; e2];
    ...
    ( EAppOp
        {
          ...
          args =
            [
              partially_evaluate_expr_for_assertion_failure_message ~eval_expr ~neg e1;
              partially_evaluate_expr_for_assertion_failure_message ~eval_expr ~neg e2;
            ];
        },
      Mark.get e )
    ...
    ( EAppOp
        {
          ...
          args =
            [
              partially_evaluate_expr_for_assertion_failure_message ~eval_expr ~neg:(not neg) e1;
            ];
        },
      Mark.get e )
  | _ -> eval_expr e
