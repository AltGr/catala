...
let scope_name ctx s = try ctx.scopes s with ScopeName.Map.Not_found _ -> s
let topdef_name ctx s = try ctx.topdefs s with TopdefName.Map.Not_found _ -> s
let struct_name ctx s = try ctx.structs s with StructName.Map.Not_found _ -> s
let enum_name ctx e = try ctx.enums e with EnumName.Map.Not_found _ -> e
