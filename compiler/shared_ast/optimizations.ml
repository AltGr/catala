...
(* beta reduction when variables not used, and for variable aliases and
   literal *)
let simplified_apply f args tys = match f with
  | EAbs { binder; _ }, _
...
let simplified_ifthenelse cond etrue efalse m =
  if Expr.equal etrue efalse then Mark.remove etrue else
...
(* builds a [EMatch] term, flattening nested matches/if-then-else: the matching arg branching are explored, and if they all lead to enum constructor literals, the surrounding match cases are inlined. Code duplication is detected and aborts the inlining. *)
let simplified_match enum_name match_arg cases mark =
  ...
  let allow_duplicate_inlining_cases =
    EnumConstructor.Map.fold (fun cons f acc ->
  ...
  let rec aux seen_constrs = function
    | EInj { cons; e; _ }, m ->
      if EnumConstructor.Set.mem cons seen_constrs
      then raise Exit; (* Abort inlining to avoid code duplication *)
      let seen_constrs =
        if EnumConstructor.Set.mem cons allow_duplicate_inlining_cases
        then seen_constrs
    ...
    | EMatch ({ cases; _ } as ematch), m ->
      let seen_constrs, cases =
        EnumConstructor.Map.fold (fun cons case (seen_constrs, acc) ->
            match case with
            | EAbs ({ binder; _ } as eabs), m ->
              let vars, body = Bindlib.unmbind binder in
              let seen_constrs, body = aux seen_constrs body in
              let binder =
                Bindlib.unbox (Expr.bind vars (Expr.rebox body))
              in
              let m = Expr.map_ty (function
            ...
            | _ ->
              assert false)
          cases (seen_constrs, EnumConstructor.Map.empty)
      in
...
let rec optimize_expr :
 ...
 fun ctx e ->
  ...
  let reduce (e : (a dcalc_lcalc, 'm) gexpr) =
    ...
    | EMatch { name; e; cases } ->
      simplified_match name e cases mark
    | EApp { f; args; tys } ->
      simplified_apply f args tys
    ...
    | EDefault { excepts; just; cons } -> (
      ...
      if value_except_count > 1 then
        ...
        | [], cond ->
          ...
          simplified_ifthenelse cond cons (EEmpty, mark) mark
        | ( [except],
            ( ( ELit (LBool false)
