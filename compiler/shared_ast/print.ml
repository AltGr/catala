...
let rec typ_gen :
 ...
 fun ~colors bctx fmt ty ->
  ...
  | TOption t ->
    Format.fprintf fmt "@[<hov 2>%a@ %a@]" base_type "optional of" (typ_gen ()) t
  | TArrow ([t1], t2) ->
...
let negated_op_to_string: type a. a Op.t -> string =
  function
  | Xor -> "="
  | Eq -> "≠"
  | Lt -> "≮"
   ...
   fun bnd_ctx colors fmt e ->
    ...
    if C.bypass fmt e then ()
      ...
      | EAppOp { op = Not, _; args = [e]; _ } ->
        (match e with
         | EAppOp { op = Not, _; args = [arg1]; _ }, _ ->
           Format.fprintf fmt "@[<hv 2>@{<crossed>%a@}@ %a@]"
             op_style "~not~"
             (paren ~context:e ~colors ~rhs:true exprc) arg1
         | EAppOp { op = (Eq|Lt|Lte|Gt|Gte|Xor) as op, _; args = [arg1; arg2]; _ }, _ ->
           Format.fprintf fmt "@[<hv 2>%a %a@ %a@]"
             (paren ~context:e ~colors ~rhs:false exprc) arg1
             negated_op op
             (paren ~context:e ~colors ~rhs:false exprc) arg2
         | e ->
           Format.fprintf fmt "@[<hv 2>%a@ %a@]" operator Not (rhs exprc) e)
...
module UserFacing = struct
  ...
  let rec value : type a.
   ...
   fun ?(fallback = fun _ _ -> invalid_arg "UserPrint.value: not a value") lang
    ...
    | EInj { name = _; cons; e } ->
      Format.fprintf ppf "@[<hov 2>%a %a@ %a@]" EnumConstructor.format cons keyword
        (match lang with Fr -> "contenu" | _ -> "content")
        (value ~fallback lang) e
