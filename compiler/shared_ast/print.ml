...
let struct_
  ...
  Format.fprintf fmt "@[<hv 2>%a %t %a %a@ %a@;<1 -2>%a@]@," keyword
...
let code_item ?(debug = false) ?name decl_ctx fmt c =
  match c with
  | ScopeDef (n, b) ->
    let n = match debug, name with true, Some n -> ScopeName.fresh [] (n, Pos.no_pos) | _ -> n in
    scope ~debug decl_ctx fmt (n, b)
  | Topdef (n, ty, e) ->
    let n = match debug, name with true, Some n -> TopdefName.fresh [] (n, Pos.no_pos) | _ -> n in
