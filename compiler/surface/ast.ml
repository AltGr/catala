...
type match_case_pattern =
  (path * uident Marked.pos) Marked.pos list
  * lident Marked.pos option
[@@deriving
  visitors
    {
      variety = "map";
    ...
    visitors
      {
        variety = "iter";
        ancestors = ["path_iter"; "lident_iter"; "uident_iter"; "Marked.pos_iter"];
...
and naked_expression =
  ...
  | ScopeCall of (path * uident Marked.pos) Marked.pos * (lident Marked.pos * expression) list
  | LetIn of lident Marked.pos * expression * expression
  | Builtin of builtin_expression
  | Literal of literal
  | EnumInject of
      (path * uident Marked.pos) Marked.pos * expression option
  | StructLit of (path * uident Marked.pos) Marked.pos * (lident Marked.pos * expression) list
