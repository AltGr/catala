...
module Commands = struct
  ...
  let print_interpretation_results options results =
    let results =
  ...
  let interpret_dcalc options link_modules optimize check_invariants ex_scope =
    ...
    let results =
      Interpreter.interpret_program_dcalc prg (get_scope_uid ctx ex_scope)
    in
    Message.emit_debug "End of interpretation";
    print_interpretation_results options results
  ...
  let interpret_lcalc
    ...
    let prg, ctx, _ =
      Passes.lcalc options ~link_modules ~optimize ~check_invariants
        ~avoid_exceptions ~closure_conversion
    in
    let scope_uid = get_scope_uid ctx ex_scope in
    Message.emit_debug "Starting interpretation (lcalc)...";
    let results = Interpreter.interpret_program_lcalc prg scope_uid in
    let results =
      List.sort (fun ((v1, _), _) ((v2, _), _) -> String.compare v1 v2) results
    in
    Message.emit_debug "End of interpretation";
