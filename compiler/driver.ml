...
let has_localised_stdlib = function
  | Global.En | Global.Fr -> true
  | _ -> false
...
let load_modules
  ...
  let find_module in_stdlib req_chain (mname, mpos) =
    ...
    let includes =
      if in_stdlib then stdlib_includes else
    ...
    match
      List.filter_map
        (fun (ext, _) ->
           File.Tree.lookup includes (mname ^ ext))
        extensions
    with
    | [] ->
      if in_stdlib then
        Message.error
          "The standard library module @{<magenta>%s@}@ could@ not@ be@ found@ at@ %a,@ please@ check@ \
        ...
        | None ->
          ...
          let seen, stdlib_use_map =
            if is_stdlib || stdlib = None then seen, Ident.Map.empty
              ...
              aux true (Mark.get use.Surface.Ast.mod_use_name :: req_chain)
                seen [stdlib_use f]
          in
          ...
          in
          let file_use_map =
            (* Uses from the stdlib are "flattened" to the parent module,
               but can still be overriden *)
  ...
  let seen, stdlib_uses =
    if stdlib = None then
      File.Map.empty, Ident.Map.empty
    else
      let stdlib_files, stdlib_use_map =
        aux true [Pos.from_info file 0 0 0 0]
          File.Map.empty [stdlib_use file]
      in
