...
type context = {
  ...
  enums : (enum_context * visibility) EnumName.Map.t;  (** For each enum, its context *)
...
let process_struct_decl ?(visibility=Public) (ctxt : context) (sdecl : Surface.Ast.struct_decl) :
    context =
...
let process_enum_decl ?(visibility=Public) (ctxt : context) (edecl : Surface.Ast.enum_decl) : context
    =
      ...
      let enums =
        EnumName.Map.update e_uid
          (fun cases ->
            ...
            | Some (fields, _) -> Some (EnumConstructor.Map.add c_uid typ fields, visibility))
          ctxt.enums
      in
      { ctxt with enums })
...
let process_scope_decl ?(visibility=Public) (ctxt : context) (decl : Surface.Ast.scope_decl) :
    context =
       ...
       output struct for convenience *)
    {
      ...
      structs =
        StructName.Map.add
          ...
          (StructField.Map.empty, visibility) ctxt.structs;
    }
  else
    let ctxt =
...
let process_name_item ?(visibility=Public) (ctxt : context) (item : Surface.Ast.code_item Mark.pos) :
    context =
...
let process_decl_item ?visibility (ctxt : context) (item : Surface.Ast.code_item Mark.pos) :
    context =
  match Mark.remove item with
...
let rec process_law_structure
    (process_item : ?visibility:visibility -> context -> Surface.Ast.code_item Mark.pos -> context)
    (ctxt : context)
  ...
  | Surface.Ast.CodeBlock (block, _, is_meta) ->
    process_code_block (process_item ~visibility:(if is_meta then Public else Private)) ctxt block
