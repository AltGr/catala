...
let form_context (surface, mod_uses) surface_modules : context =
  let rec process_modules ctxt mod_uses =
    ...
    Ident.Map.fold (fun _alias m ctxt ->
        ...
        | None ->
          ...
          let ctxt = { ctxt with local = { ctxt.local with used_modules = mod_uses } } in
      ...
      mod_uses ctxt
  in
  let ctxt = { empty_ctxt with local = { empty_module_ctxt with used_modules = mod_uses } } in
  let ctxt = process_modules ctxt mod_uses in
  let ctxt =
    List.fold_left
  ...
  let noconflict _ _ _ = assert false in
  let sub_constructor_idmap, sub_field_idmap =
    Ident.Map.fold (fun _ m (cmap, fmap) ->
        let lctx = ModuleName.Map.find m ctxt.modules in
        let cmap =
          Ident.Map.union
            (fun _ enu1 enu2 -> Some (EnumName.Map.union noconflict enu1 enu2))
            cmap lctx.constructor_idmap
        in
        let fmap =
          Ident.Map.union
            (fun _ str1 str2 -> Some (StructName.Map.union noconflict str1 str2))
