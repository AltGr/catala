    ...
    | S.WildCard match_case_expr -> (
      ...
      | Some e_uid ->
        ...
        let missing_constructors =
          EnumName.Map.find e_uid ctxt.Name_resolution.enums |> fst
  ...
  let module_topdefs =
    TopdefName.Map.update id
      (fun def0 ->
        ...
        | None, eopt -> Some { Ast.topdef_expr = eopt; topdef_visibility; topdef_type = typ }
        | Some def0, eopt -> (
          let err msg =
            Message.error
              ~extra_pos:
          ...
          in
          if not (Type.equal def0.Ast.topdef_type typ) then err "Conflicting type definitions"
          else
            match def0.Ast.topdef_expr, eopt with
            | None, None -> err "Multiple declarations"
            | Some _, Some _ -> err "Multiple definitions"
            | Some _ as topdef_expr, None -> Some { Ast.topdef_expr; topdef_visibility; topdef_type = typ }
            | None, (Some _ as topdef_expr) -> Some { Ast.topdef_expr; topdef_visibility; topdef_type = def0.topdef_type }))
...
let translate_program (ctxt : Name_resolution.context) (surface : S.program) :
      ...
      (fun mctx ->
        {
          ...
          Ast.module_topdefs =
            Ident.Map.fold
              (fun _ name acc ->
                 let topdef_type, topdef_visibility = TopdefName.Map.find name ctxt.Name_resolution.topdefs in
          ...
          Ident.Map.fold
            (fun _ m acc ->
               ...
               let mhash =
                 ...
                 (* We could include the hashes of submodule interfaces in the hash of the module ; however, the module is already responsible for checking the consistency of its dependencies upon load, and that would result in harder to track errors on mismatch. *)
               in
  ...
  let program_module_name =
    surface.Surface.Ast.program_module_name |> Option.map @@
    fun id ->
  ...
  let rec process_structure (prgm : Ast.program) (item : S.law_structure) :
    ...
    | S.LawHeading (_, children) ->
      List.fold_left
        ...
        prgm children
    | S.CodeBlock (block, _, is_meta) -> process_code_block ctxt prgm is_meta block
  ...
  { desugared with
    Ast.program_module_name =
      desugared.Ast.program_module_name |> Option.map @@ fun (mname, _) ->
      mname, Ast.Hash.module_binding ~root:true mname desugared.Ast.program_root;
  }
