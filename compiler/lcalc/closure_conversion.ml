 ...
 fun ctx e ->
  ...
  | (EVar _ | EExternal _) as e -> (
    let body, (free_vars, fty) =
      ...
      | EVar v -> (
        ( Bindlib.box_var v,
          match Option.map Type.unquantify @@ Var.Map.find_opt v ctx.globally_bound_vars with
      ...
      | EExternal { name = External_value td, _ } as e ->
        ( Bindlib.box e,
          ( Var.Map.empty,
            match Type.unquantify @@ fst @@ TopdefName.Map.find td ctx.decl_ctx.ctx_topdefs with
