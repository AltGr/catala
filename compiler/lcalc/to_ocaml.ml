...
let rec format_expr (ctx : decl_ctx) (fmt : Format.formatter) (e : 'm expr) :
  ...
  | EFatalError er ->
    Format.fprintf fmt "raise@ (Error (%a, [%a]))" Print.runtime_error
...
let check_and_reexport_used_modules ppml ppi ~hashf modules =
  List.iter
    (fun (m, intf_id) ->
      pp [ppml]
        "@[<hv 2>let () =@ @[<hov 2>match Catala_runtime.check_module %S \"%a\"@ \
         with@]@,\
         | Ok () -> ()@,\
...
let format_module_registration ctx fmt exports modname hash is_external =
  ...
  Format.fprintf fmt "Catala_runtime.register_module \"%a\"" ModuleName.format modname;
  Format.pp_print_space fmt ();
  Format.pp_open_vbox fmt 2;
  Format.pp_print_string fmt "[ ";
  Format.pp_print_list
...
let format_program
  ...
  check_and_reexport_used_modules ppml ppi ~hashf
    (List.map (fun (m, intf) -> m, intf.intf_id) (ModuleName.Map.bindings p.decl_ctx.ctx_modules));
