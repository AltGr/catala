...
let collect_monomorphized_instances (prg : typed program) :
  ...
  in
  let acc =
    Scope.fold_exprs prg.code_items
      ~init:empty_instances
      ~f:(fun acc e typ -> collect_typ (collect_expr e acc) typ)
  in
  EnumName.Map.fold
...
let program (prg : typed program) :
  ...
  let ctx_enums =
    EnumName.Map.remove Expr.option_enum decl_ctx.ctx_enums
  in
