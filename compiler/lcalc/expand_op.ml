...
let rec resolve_eq ctx pos ty args m =
  ...
  | TOption ty ->
    ...
    Expr.ematch ~name:Expr.option_enum ~e:arg1 m
      ~cases:(EnumConstructor.Map.of_list [
          Expr.none_constr,
          Expr.thunk_term
            (Expr.ematch ~name:Expr.option_enum ~e:arg2 m
               ~cases:(EnumConstructor.Map.of_list [
                   Expr.none_constr, Expr.thunk_term (Expr.elit (LBool true) m);
                   Expr.some_constr,
                   Expr.make_ghost_abs
                     [Var.make "_"]
                     (Expr.elit (LBool false) m)
          ...
          Expr.make_ghost_abs [v1]
            (Expr.ematch ~name:Expr.option_enum ~e:arg2 m
               ~cases:(EnumConstructor.Map.of_list [
                   Expr.none_constr, Expr.thunk_term (Expr.elit (LBool false) m);
                   Expr.some_constr,
                   Expr.make_ghost_abs [v2]
                     (resolve_eq ctx pos ty [Expr.evar v1 (Expr.with_ty m ty);
                                             Expr.evar v2 (Expr.with_ty m ty)] m)
                     [ty] pos
              ]))
            [ty]
            pos
