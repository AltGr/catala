...
val copy_dir : ?filter:(t -> bool) -> ?newer_only:bool -> src:t -> dst:t -> unit -> unit
(** Recursively copy a directory with its contents using [copy]. [filter] is
    ...
    links or attributes, or file reading errors. If [newer_only] is set to [true], on files for which the destination already exists, the copy is only done if it is older than the source *)
