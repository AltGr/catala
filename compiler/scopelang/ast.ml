...
let rec locations_used (e : 'm expr) : LocationSet.t =
  ...
  | EAbs (binder, _), _ ->
    ...
    locations_used body
  | e ->
    Expr.shallow_fold (fun e -> LocationSet.union (locations_used e)) e
      LocationSet.empty
    ...
    ScopeMap.fold
      (fun scope_name scope_decl ->
         let input_vars, output_vars =
           ScopeVarMap.fold (fun var (typ, io) (input, output) ->
               (if Marked.unmark io.io_input <> NoInput then ScopeVarMap.add var typ input else input),
               (if Marked.unmark io.io_output then ScopeVarMap.add var typ output else output)
             )
           scope_decl.scope_sig (ScopeVarMap.empty, ScopeVarMap.empty)
         in
