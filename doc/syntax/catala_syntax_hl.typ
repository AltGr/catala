...
#let style = (
    ...
    literal: (fill: palette.bluegreen),
    ...
    uid: (fill: palette.azur, weight: "bold"),
    field: (style: "oblique"),
...
#let show-catala-fr-code(txt) = {
    ...
    show regex("\|[0-9]{4}-[0-9]{2}-[0-9]{2}\|"): override.with(style.literal + (weight: "medium"))
    show regex("--\s+(n'importe\s+quel|[\pL\d_']+)"): txt => {
        show "--": override.with(style.punctuation + (style: "normal", weight: "medium"))
        show regex("\b\p{Ll}[\pL\d_']*"): text.with(..style.field)
        show regex("n'importe\s+quel"): override.with(style.keyword + (style: "normal"))
        txt
    }
    show regex("\b(contexte|entrée|résultat|interne|état|date\s+arrondi(\s+dé)?|(dé)?croissant|soit|dans|dépend\s+de|contenu|sous\s+condition|condition|donnée|conséquence|rempli|égal\s+à)\b"): override.with(style.keyword + (style: "normal", weight: "medium"))
    show regex("\bdonnée\s+\w+\b"): txt => {
        show regex("\s+.*"): text.with(..style.field)
        txt
    }
    show regex("\b((liste\s+de)|(ou\s+si\s+liste\s+vide\s+alors)|(combine\s+tout))\b"): override.with(style.keyword + (weight: "medium"))
...
#let show-catala-en-code(txt) = {
    ...
    show regex("\b(context|input|output|internal|state|date\s+round|(de|in)?creasing|let|in|depends\s+on|content|under\s+condition|condition|data|consequence|fulfilled|equals)\b"): override.with(style.keyword + (style: "normal", weight: "medium"))
    show regex("\bdata\s+\w+\b"): txt => {
        show regex("\s+.*"): text.with(..style.field)
        txt
    }
    show regex("\b((list\s+of)|(or\s+if\s+list\s+empty\s+then)|(combine\s+all))\b"): override.with(style.keyword + (weight: "medium"))
    ...
    show regex("~~\S+~~"): txt => {
        show "~~": []
        show text: txt => strike(text(..style.keyword, weight: "medium", txt))
        txt
    }
    show regex("#.*"): override.with(style.comment)
    txt
