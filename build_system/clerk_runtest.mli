...
val run_inline_tests :
  ...
  out:File.t option ->
  File.t -> unit
(** [run_inline_tests ~catala_exe ~catala_opts ~test_flags ~report file] runs the tests in
...
run: test_file -> test_out + report

reset: test_file out -> (copy)
