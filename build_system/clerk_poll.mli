...
(** Locates the directory containing the runtimes to link to (in one subdirectory per backend) *)

val stdlib_dir : File.t Lazy.t
(** Locates the directory containing the standard library, including external implementations. Equal to [runtime_dir] on a standard installation. *)
