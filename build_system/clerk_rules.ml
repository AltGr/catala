...
let backend_from_config = function
  ...
  | Clerk_config.C ->
    (module Clerk_backend.C.Backend : Clerk_backend.Backend.S)
  | Clerk_config.Java ->
...
let gen_build_statements
  ...
  let backend_sources =
    if item.extrnal then
      ...
      let inputs = [Nj.Expr.Word catala_src] in
      let implicit_in =
  ...
  let expose_module =
    ...
    | Some m when item.is_stdlib || List.mem dir include_dirs ->
      ...
      in
      Nj.build "phony"
        ~outputs:[Word (modname ^ "@src")]
        ~inputs:[Word catala_src]
      :: backends_module
    | _ -> []
  in
  let tests_rules =
                 ...
                 (fun m ->
                   Nj.Expr.Word
                     (Ninja.modfile ~backend:"ocaml" same_dir_modules
                        "@ocaml-module" m))
                 modules)
          ~outputs:[Word (catala_src ^ "@test"); Word (catala_src ^ "@out")];
      ]
  in
...
let dir_test_rules dir subdirs items =
  ...
  List.to_seq
    [
      Nj.comment "";
      Nj.build "dir-tests"
        ~outputs:[Word ((Var.(!builddir) / dir) ^ "@test")]
        ~inputs:(List.map (fun s -> Nj.Expr.Word s) inputs)
        ~vars:
  ...
  if tests then
    pp
      (Nj.build "phony" ~outputs:[Word "test"]
         ~inputs:[Word File.(Var.(!builddir / ".@test"))]);
  Seq.Nil
