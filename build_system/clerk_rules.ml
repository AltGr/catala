...
let base_bindings ~autotest ~enabled_backends ~config =
  ...
  if List.mem C enabled_backends then
    let dirs = lazy (
      ...
      if String.ends_with runtime ~suffix:"_build/default/runtimes/c"
      then
    ...
    [
      ...
      def Var.runtime_c_libs
        (lazy
          (List.map (fun d -> "-L"^d) (Lazy.force dirs) @ ["-lcatala_runtime"; "-lgmp"]));
      def Var.c_flags
        (lazy
          [
            "-std=c89";
      ...
      def Var.c_include
        (lazy
          (List.map (fun d -> "-I"^d) (Lazy.force dirs) @ includes ~backend:"c" ()));
    ]
  else []
