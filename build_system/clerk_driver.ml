...
let ninja_build_targets
  ...
  let item_exec_target ?backends:explicit_backends it =
    let is_in_include () =
      (* deprecate this ? *)
      List.mem
        (File.dirname it.Scan.file_name)
        config.Clerk_cli.options.global.include_dirs
    in
    let backends =
    ...
    List.concat_map
      (fun backend ->
        let t =
          match it.Scan.module_def with
          | Some (m, _) when is_in_include () ->
            Printf.sprintf "%s@%s-module" m
...
let run_targets
  ...
  let run_target ((item, backend, target) as test_target) =
    ...
    | `Interpret ->
      ...
      in
      let catala_flags =
        Var.get_var info.Clerk_rules.var_bindings Var.catala_flags
        @ (match scope with
          | None -> []
      ...
      in
      let exec = Var.get_var info.Clerk_rules.var_bindings Var.catala_exe in
...
let build_cmd : int Cmd.t =
  let run
    ...
    (* todo: advertise other built targets with their location in _build *)
    (* todo: build specific artifacts from direct_targets that need linking *)
    List.iter
