...
let build_clerk_target
  ...
  let install_targets, all_modules_deps =
    ...
    in
    let install_targets =
      List.map (fun ((_item, _target, bk), file) -> bk, file)
        all_target_files
    in
    let all_targets, install_targets =
      ...
      if List.mem Config.OCaml target.backends then
        let lib = build_dir / backend_subdir OCaml / target.tname -.- "cmxa" in
        let inputs =
          List.map (fun module_item ->
              build_dir
              / dirname module_item.Scan.file_name
              / backend_subdir OCaml
              / (Option.get module_item.module_def |> Mark.remove) -.- "cmx"
            )
            all_modules_deps
        in
        Nj.format nin_ppf
  ...
  if target.Config.include_sources then
    all_modules_deps |> List.map (fun it -> it.Scan.file_name)
