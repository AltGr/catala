...
class Date:
    ...
    def __init__(self, value: Union [Tuple[Integer, Integer, Integer], dates.Date], pos: SourcePosition | None = None):
        try:
...
@dates.Infix
def up(l : Date, p : Duration) -> Date:
    if not (isinstance(l, Date) and isinstance(p, Duration)): raise TypeError("+up+ requires a date and a duration")
    return l.__add__(p, dates.DateRounding.RoundUp)

@dates.Infix
def down(l : Date, p : Duration) -> Date:
    if not (isinstance(l, Date) and isinstance(p, Duration)): raise TypeError("+down+ requires a date and a duration")
    return l.__add__(p, dates.DateRounding.RoundDown)
...
class Duration:
    ...
    def __init__(self, value: Union [Tuple[Integer, Integer, Integer], dates.Period]):
